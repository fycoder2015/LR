...
public class UserPhoneTools {
	...
	/**
	 * 短信登录时 根据 phonenumber和 验证码captchacode 判断匹配
	 * 验证码的修改时间为 Constants.SMS_Gap_Time*分钟  
	 * 
	 * -------------------------------------
	 * 功能描述：
	 ...
	 *  验证码超时 需要重新获取验证码 
	 */
	public int checkPhoneInSmsLogin(String phonenumber ,String captchacode ) {
		int returncode =  0 ;
		int errcode = -1 ;
		String errmsg = "比对不成功";
		int successcode = 1 ;
	 ...
	 * 
	 */
	public int genCaptchacodeByPhoneInSmslogin(String phonenumber ) {
		//GeneralResponse gp = new GeneralResponse();
		int returncode =  0 ;
		int errcode = -1 ;
		String errmsg = "用户手机号未激活或是存在问题";
		int successcode = 1 ;
		String successmsg = "短信发送成功";
		int senderr = 0;
		String senderrmsg="短信发送失败";
		...
		Phonenumber p = accountService.findUserPhoneByPhonenumberIsActivity(phonenumber);	
		//找到 已激活的用户的手机对象 
		if (p == null){			
			//gp.setRetCode(errcode);
			//gp.setRetInfo(errmsg);
